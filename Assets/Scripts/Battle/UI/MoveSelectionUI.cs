using PKMNUtils.GenericSelectionUI;
using System.Collections;
using System.Collections.Generic;
using TMPro;
using System.Linq;
using UnityEngine;
using UnityEngine.UI;

public class MoveSelectionUI : SelectionUI<TextSlot>
{
    [SerializeField] List<TextSlot> moveTexts;

    [SerializeField] TMP_Text ppText;
    [SerializeField] TMP_Text typeText;

    private void Start()
    {
        SetSelectionSettings(SelectionType.Grid, 2);
        UpdateSelectionInUI();
    }

    List<Move> _moves;
    public void SetMoves(List<Move> moves)
    {
        _moves = moves;

        selectedItem = 0;

        SetItems(moveTexts.Take(moves.Count).ToList());

        for (int i = 0; i < moveTexts.Count; ++i)
        {
            if (i < moves.Count)
                moveTexts[i].SetText(moves[i].Base.Name);
            else
                moveTexts[i].SetText("-");
        }

        UpdateSelectionInUI();
    }

    public override void UpdateSelectionInUI()
    {
        base.UpdateSelectionInUI();

        for (int i = 0; i < moveTexts.Count; ++i)
        {
            if (i < _moves.Count)
            {
                moveTexts[i].GetComponent<TextMeshProUGUI>().color = (i == selectedItem)
                    ? GlobalSettings.i.HighlightedColor
                    : Color.black; // Assuming black is the original color, adjust if necessary
            }
        }

        var move = _moves[selectedItem];

        ppText.text = $"PP {move.PP} / {move.Base.PP}";
        typeText.text = move.Base.Type.ToString();

        if (move.PP == 0)
            ppText.color = Color.red;
        else
            ppText.color = Color.black;
    }
}